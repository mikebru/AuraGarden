...
CGINCLUDE
	...
	half4 frag( v2f i ) : SV_Target
	{
		...
		half4 screenWithOffset = i.screenPos;// +half4(_EyeOffset, 0, 0, 0);
...
#endif
		//screenWithOffset += distortOffset;
		...
		#endif
			//refl2Refr = half4(0, 0, 0, 1);
			//rtRefractions = half4(0, 0, 0, 1);
			float lerpValue = (refl2Refr + 1) / 2;
			baseColor = lerp(lerp(rtRefractions, baseColor, baseColor.a), reflectionColor, refl2Refr);
			//baseColor = lerp(lerp(rtRefractions, baseColor, baseColor.a), reflectionColor, lerpValue);
			//baseColor = reflectionColor;// *_EyeOffsetColor;
		...
		return baseColor;
		//return tex2Dproj(_ReflectionTex, UNITY_PROJ_COORD(screenWithOffset));
	}
