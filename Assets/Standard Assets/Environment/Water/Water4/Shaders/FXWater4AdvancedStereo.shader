...
Shader "FX/Water4AdvancedStereo" {
Properties {
	_ReflectionTex("Internal reflection", 2D) = "white" {}
	//_RefractionTex ("Internal refraction", 2D) = "white" {}
	_EyeOffset("Eye Offset", Range(0.0, 1.0)) = 1.0
	_EyeOffsetColor("EyeOffset color", COLOR) = (1, 1, 1, 1)
	...
	_BumpDirection ("Bump Direction & Speed", Vector) = (1.0 ,1.0, -1.0, 1.0)
		...
		_SinglePassStereoRefractionDistortionScale("SinglePassStereo Refraction Distortion Scale", Range(0.0, 100.0)) = 2
		_DistortionScale ("Distortion Scale", Range (0.0, 100.0)) = 5
...
#include "WaterStereoInclude.cginc"
	...
	uniform float4 _EyeOffsetColor;
	...
	uniform float _SinglePassStereoRefractionDistortionScale;
	...
	half4 frag( v2f i ) : SV_Target
	{
		...
		half4 distortOffset = half4(worldNormal.xz * REALTIME_DISTORTION * _DistortionScale, 0, 0);
		//half4 distortOffset = half4(VERTEX_WORLD_NORMAL.xz * REALTIME_DISTORTION * _DistortionScale, 0, 0);
		half4 screenWithOffset = i.screenPos + distortOffset;
#ifdef DEBUG_WATER_REFLECTION
		screenWithOffset = i.screenPos;
#endif
		...
		{
			screenWithOffset.x = (screenWithOffset.x * 0.5f);
		}
		else if (unity_CameraProjection[0][2] > 0)
		{
			screenWithOffset.x = (screenWithOffset.x * 0.5f) + (screenWithOffset.w * 0.5f);
		}
#endif
...
#else
		half4 grabWithOffset = screenWithOffset + half4(worldNormal.xz * REALTIME_DISTORTION * _SinglePassStereoRefractionDistortionScale, 0, 0);
		half4 rtRefractionsNoDistort = tex2Dproj(_RefractionTex, UNITY_PROJ_COORD(i.screenPos));
#endif
		...
		#endif
#ifdef DEBUG_WATER_REFRACTION
			reflectionColor = half4(0, 0, 0, 0);
#endif
